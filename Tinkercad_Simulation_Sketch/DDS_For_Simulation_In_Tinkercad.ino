...
//                              M = tuning word
//                     sampleRate = 44100 Hz (one sample sent out every 22.675737 uSec)
...
int phInc;                           // dds phase increment
unsigned long phAcc;                 // dds phase accumulator
unsigned long tuningWord;            // dds tuning word (M)

byte tableAddrWidth;                 // number of bits needed to address all elements of sample data table
                                     // eg. if table contains 512 samples addressable from 0 to 511,
                                     // 511 = 0b111111111 = 9 bit wide address number

int dacData = 0;                     // dac output data

// variables for debug printing in Arduino simulator
byte nSamples = 1;                   // how many samples were taken from data table so far this cycle
int phIncOld = 0;                    // previous value of phInc

void setup()
{
  // Serial monitor is not used outside simulation to avoid timer/timing issues
  Serial.begin(9600);         
  Serial.println("\nDDS Test");
  
  tableAddrWidth = numBits(samplesPerCycle-1);  // to access the highest data sample in the table,
                                                // how many bits are needed to count up to that 
                                                // highest table position? eg. 511 is a 9 bit num
  // set output frequency in Hz
  fOut=86.0;                           
  tuningWord=pow(2,32)*fOut/sampleRate;         // DDS tuning word for target frequency
  
  Serial.print("f Out (Hz):");
  Serial.println(fOut);
  
  Serial.print("tW:");
  Serial.println(tuningWord);
  
  Serial.print("# samples required for one wave cycle:");
  Serial.println(pow(2,32)/tuningWord);
}

void loop()
{
  phAcc+=tuningWord;         // increment phase accumulator by the tuning word amount 
  phInc=phAcc >> (32 - tableAddrWidth);         
                             // use highest 9 bits for phase increment, which is the 
                             // position in the sine data table to get a sample from
                             // (the data table contains 512 data points and 
                             //  512 DEC requires a 9 bit wide BIN number to address all samples)
                        
  dacData=sineData[phInc];   // send current sine data sample to the DAC
  
  //-------------debug serial monitor output for simulation-----------------------------------------------
  if (phInc < phIncOld) {     // if we started a new wave cycle, reset the sample counter to track visually  
    nSamples = 1;  
    Serial.println("End cycle");    
  }  
  phIncOld = phInc;           // the phInc data just used is now old so use for historical value tracking

  Serial.print(" n:");
  Serial.print(nSamples);
  Serial.print(" pA bin:");
  Serial.print(phAcc,BIN);
  Serial.print(" dec:");
  Serial.print(phAcc);  
  Serial.print(" pI:");
  Serial.print(phInc);
  ...
  nSamples+=1;               // count the number of samples used from the data table so far in cycle
