...
void setup()
{
  ...
  tuningWord2 = pow(2, 32) * fOut2 / sampleRate;  // DDS tuning word for target frequency
  ...
  // configure Due ADC for 12 bits to read 0-4095 instead of default 10 bit 0-1023
...
void loop()
{
  ...
  /*
    ...
    for (int i = 0; i < 4000; i++) {
      ...
      tuningWord = pow(2, 32) * fOut / sampleRate;        // DDS tuning word for target frequency
      fOut2 = 4000 - i;                                   // set output frequency in Hz
      tuningWord2 = pow(2, 32) * fOut2 / sampleRate;      // DDS tuning word for target frequency
      delay(5);
    }
  ...
  */
  ...
  // set frequency of both oscillators with pots on ADC 0 and 1
  // frequency is 0 to 4095 Hz
  fOut = analogRead(0);
  tuningWord = pow(2, 32) * fOut / sampleRate;
...
void TC4_Handler()
{
  ...
  int dacData = (sineData[phInc] >> 1) + (sineData[phInc2] >> 1);
  dacc_write_conversion_data(DACC_INTERFACE, dacData);
